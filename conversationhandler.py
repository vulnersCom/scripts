...
#
# Tuned by isox@vulners.com for cross-process conversation handling with Django Cache wrapper
#
...
from django.core import cache as cacheController
import hashlib
...
class ConversationHandler(Handler):
    ...
    def __init__(self,
        ...
        # Use session cache to hold conversation states
        self.cacheWrapper = cacheController.caches['telegramConversations']

    def makeKey(self, tupleData):
        hashdata = hashlib.md5()
        hashdata.update('_'.join(["%s" % element for element in tupleData]).encode('utf-8'))
        return "TG_CONV_%s" % hashdata.hexdigest()
    ...
    def check_update(self, update):
        ...
        state = self.cacheWrapper.get(self.makeKey(key))
        # Resolve promises
    ...
    def update_state(self, new_state, key):
        if new_state == self.END:
            self.cacheWrapper.delete(self.makeKey(key))
        elif isinstance(new_state, Promise):
            stateTuple = (self.cacheWrapper.get(self.makeKey(key)), new_state)
            self.cacheWrapper.set(self.makeKey(key), stateTuple, timeout=90)

        elif new_state is not None:
            self.cacheWrapper.set(self.makeKey(key), new_state, timeout=90)
